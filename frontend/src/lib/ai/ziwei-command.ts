...
遇到上述显性笔误时，可在“完整依据”对应位置备注“按自纠规则归位”，不得强行圆说，也不得写成资料矛盾或逻辑存疑。
...
【输出结构：必须严格按以下 Markdown 顺序】

# 快速浏览

## 一句话结论
- 用1–2句话概括命盘主轴、当前阶段和本次分析重点，直接说明主要优势与主要风险。
- 另起一行写“总体判断把握：高／中／低”，并用一句话说明不确定性来源。

## 命身主轴
- 用不超过3条短句说明命宫、身宫、命主、身主及生年四化形成的核心结构。
- 命宫无主星时说明借看迁移宫；不要在这里逐宫罗列全部星曜。

## 运势节奏
- 先写当前大限的宫位、主导星曜与十年主题，再结合本次专项宫位概括近、中、远期取舍。
- “流年”专项须覆盖规范数据可支持的未来五年；其他专项只写与主题直接相关的时间线索，不制造确定事件。

## 行动建议
- 给出3–4条与本次重点直接相关、可执行、低风险的建议；每条先写动作，再简述对应的宫位或运限依据。
...
## 输入核验
- 完整列出姓名、性别、出生时间、地点、四柱、阴阳性别、五行局、命宫、身宫、流年命宫、小限命宫、起运及真实缺失项。
- 若触发数据自纠偏，明确写出采用的规范字段和“按自纠规则归位”，不把显性标签差异扩写成命理结论。

## 命盘骨架
- 完整说明命宫、身宫、命主、身主、生年四化、三方四正和对宫如何形成命盘主轴。
- 每个小项使用短块表达“盘面事实／现代含义／适用边界”，保留有独立判断价值的主星亮度、辅杂曜、四化、空宫与会照证据。

## 专项宫位
- 按【当前选择】分析对应的重点宫位，并说明它们与命宫、身宫、福德宫、迁移宫及三方四正的关系。
- 全项分析覆盖命格、事业财帛、婚恋人际、健康作息；专项分析聚焦主题，但不得省略会改变结论的反向证据和补偿条件。

## 运限与时间
- 先按十二宫“大限年龄”定位当前大限，再以十年运干支作为年份节奏参考；不得颠倒二者主次。
- 结合大限、小限、流年命宫及相关三方四正说明阶段变化、机会、风险与近中远期取舍，不把传统象征写成必然事件。

## 证据与边界
- 只保留2–3项核心结论，依次编号为“结论A／结论B／结论C”。
- 每项固定列出“支持证据／反向证据／综合判断／判断把握”，证据必须引用具体宫位、星曜、四化、三方四正或大限数据。
- 最后说明哪些信息不能仅凭命盘确定，并保留法律、医疗、投资、心理咨询等现实边界。

【去重、排版与篇幅】
1. 所有独立的命盘事实、判断依据、反向证据、适用边界和行动建议都必须保留；同一结论只完整表达一次，其余位置用“见结论A／B／C”建立关联，不换词重复扩写。
...
4. 只在总体判断和2–3项核心结论中说明判断把握；宫位、星曜和运限小项不逐条重复“置信度”。
5. 必须解释紫微术语，语言理性、专业、清楚、可执行；取消宿命论、恐吓式表达和空泛标签。
`;
}
