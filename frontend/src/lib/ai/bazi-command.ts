...
"",
##"【7. 输出结构：必须严格按以下 Markdown 顺序】",

# 快速浏览

## 一句话结论
- 用1–2句话概括命局核心矛盾、主要生机和本次专项倾向，先说明风险，再说明可用条件。
- 可以使用寒暖、燥湿、克泄、枯荣、通塞等传统意象，但不得脱离盘面，也不得使用空洞吉利话。
- 另起一行写“总体判断把握：高／中／低”，并说明时柱、大运或流年等不确定性来源。

## 命局主轴
- 用不超过3条短句说明“日主旺衰 → 月令格局 → 病药关系 → 喜忌主线”。
- 明确写出日主是否得令、得地、得助，以及主用神、喜神、条件用神和忌神；不在这里重复全部藏干和十神。

## 运势节奏
- 用一行短链路概括“当前大运 → 当前流年 → 未来两年”，注明年份、干支和攻守倾向。
- 只概括与本次分析重点直接相关的机会、风险和触发条件；完整逐运、逐年证据放到“完整依据”。

## 行动建议
- 给出3–5条现实可执行、低风险的建议，覆盖本次专项所需的职业、财务、关系、学习或健康方向。
- 颜色、方位、行业等传统五行建议只能作为偏好参考，须同时给出现实核验方式，不得替代专业决策。
...
## 输入核验
- 完整列出姓名、性别、出生时间、出生地、真太阳时使用情况、四柱、当前流年、大运数据范围及真实缺失项。
- 严格执行数据完整性分层：一级缺失停止；二级、三级缺失标明具体影响，不得补造时柱、大运或流年。

## 旺衰与格局
- 依次说明季节月令、日主五行、得令、得地、得助、通根、透干、帮扶、克泄耗、十二长生和身强弱结论。
- 说明月令取格、天干合化或合绊、从格／专旺／化气条件、天干生扶克战，以及程序已提供的六合、冲、刑、害、半合、三合、三会。
- 提炼“何者为病、何者为药”，每项使用“盘面事实／现代含义／适用边界”短块表达，不把单一五行数量当成结论。

## 喜忌与专项
- 按“扶抑安全线 → 格局 → 通关 → 调候 → 平衡复核”的顺序说明用神、喜神、条件用神和忌神。
- 木、火、土、金、水分别写明对应十神、强弱程度、喜／用／忌／闲性质及依据；全项逐一分析，专项重点展开相关2–3项，其余合并但不得删除会改变结论的证据。
- 按${focus}主题分析相关事业、财富、婚恋、子女、六亲、健康或学业结构，保留十神组合、日支／配偶宫及刑冲合害等支持与反向证据。

## 大运流年
- 全项分析程序提供的至多8步大运；专项重点分析3–5步相关大运，其余逐步简述。每步写年龄／年份、干支、用神是否到位、五行变化、主要机会、主要风险和综合倾向。
- 当前流年为${currentYearGanZhi}(${currentYear}年)，并分析${nextYear1GanZhi}(${currentYear + 1}年)、${nextYear2GanZhi}(${currentYear + 2}年)。先看大运十年基调，再看流年触发原局冲合，最后说明与本次专项相关的近中期节奏。
- 如保留事业、财运、感情、健康、贵人评分，必须逐项说明依据并明确评分只是传统象征量表；数据不足时省略评分，不制造精确性。
- 大运或流年数据到末端即止，明确说明剩余信息不足，不得编造未提供的运程。

## 证据与边界
- 只保留2–3项核心结论，依次编号为“结论A／结论B／结论C”。
- 每项固定列出“支持证据／反向证据／综合判断／判断把握”，涵盖旺衰、格局、喜忌、专项与运势之间的交叉验证。
- 最后说明传统命理不能确定的现实信息；财运不得写成收益保证，健康不得写医疗诊断，婚姻不得绝对化，法律、医疗、投资等事项须结合现实资料和专业意见。

##"【8. 去重、文风、排版与篇幅】",
- 所有独立的盘面事实、判断依据、反向证据、适用边界和行动建议都必须保留；同一结论只完整表达一次，其余位置用“见结论A／B／C”建立关联，不换词重复扩写。
- 快速浏览摘要约 300–500 个中文字符；完整依据按实际证据展开，不设最低字数。禁止为压缩篇幅省略关键证据，也禁止为凑篇幅重复结论。
- 每段控制在3–4行，优先使用短句和项目符号。禁止使用 Markdown 表格，避免手机横向滚动；不要使用 HTML <details>，外部界面不保证支持折叠。
- 核心判断可使用克制的传统意象；分析说明使用白话并解释首次出现的术语；建议使用现代语言。不得故弄玄虚或堆砌意象。
- 只在总体判断和2–3项核心结论中说明判断把握；五行、大运和流年小项不逐条重复“置信度”。
    `
  ].join("\n");
}
