...
function getCachedAlmanac(date, dayMaster) {
    ...
    if (cached)
        almanacCache.delete(key);
    return undefined;
}
function setCachedAlmanac(date, dayMaster, data) {
    const key = `${date}:${dayMaster || 'none'}`;
...
function parseAlmanacDate(date) {
...
export async function calculateDailyAlmanac(input) {
    // 解析日期为本地时间，避免 UTC 偏移
    let targetDate;
    if (input.date) {
