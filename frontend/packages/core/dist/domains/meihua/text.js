import { traditionalYaoName } from '../liuyao/calculate.js';
import { normalizeDetailLevelBinary } from '../../shared/render-utils.js';
function formatWallClockLabel(value) {
    return value.replace('T', ' ');
}
function formatGanZhiLabel(result) {
...
function formatTrigramRole(label, trigram) {
    return `${label}(${trigram.name}${trigram.element})`;
}
function formatHexagramLine(label, hexagram, options) {
    const extras = [];
...
function buildRelationExpression(relation, body, other) {
...
function buildStageExpressions(result) {
...
export function renderMeihuaCanonicalText(result, options = {}) {
    ...
    const lines = ['# 梅花易数主证据', '', '## 起卦与环境'];
    const showResolvedMode = !!(result.castMeta.resolvedMode && (result.castMeta.methodFamily === 'extended'
    ...
    if (detailLevel === 'full' && result.mainHexagram.xiangCi)
    ...
    if (result.nuclearHexagram) {
        ...
        if (detailLevel === 'full' && result.nuclearHexagram.guaCi)
            lines.push(`- 互卦卦辞: ${result.nuclearHexagram.guaCi}`);
        if (detailLevel === 'full' && result.nuclearHexagram.xiangCi)
    ...
    if (result.changedHexagram) {
        ...
        if (detailLevel === 'full' && result.changedHexagram.xiangCi)
    ...
    if (detailLevel === 'full') {
        if (result.seasonalState.bodyMutual)
            lines.push(`- 体互月令: ${result.seasonalState.bodyMutual}`);
        if (result.seasonalState.useMutual)
            lines.push(`- 用互月令: ${result.seasonalState.useMutual}`);
        if (result.seasonalState.changed)
            lines.push(`- 变卦月令: ${result.seasonalState.changed}`);
    }
    lines.push('');
    lines.push('## 阶段推演');
