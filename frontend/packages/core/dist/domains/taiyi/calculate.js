...
function ensureMinute(minute, mode) {
    if (mode === 'minute') {
...
function formatSolarDateTime(year, month, day, hour, minute) {
    return `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')} ${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}`;
}
function buildTaiyiStarSnapshot(star, scale, scaleLabel) {
    return {
        scale,
        scaleLabel,
...
function buildMinuteRefinement(hourStar, minute) {
...
function buildDatetimeContext(lunar, year, month, day, hour, minute) {
    return {
...
function buildDerivedIndicators(primaryStar, context) {
    const favorableSignals = [];
    const cautionSignals = [];
    if (primaryStar.taiyiType === '吉神')
        favorableSignals.push(`主星 ${primaryStar.taiyiName} 属吉神`);
    if (primaryStar.taiyiType === '凶神')
        cautionSignals.push(`主星 ${primaryStar.taiyiName} 属凶神`);
    if (context.tianShenLuck === '吉')
        favorableSignals.push(`日天神 ${context.tianShen} 属吉`);
    if (context.tianShenLuck === '凶')
        cautionSignals.push(`日天神 ${context.tianShen} 属凶`);
    if (context.xiuLuck === '吉')
        favorableSignals.push(`宿曜 ${context.xiu} 为吉`);
    if (context.xiuLuck === '凶')
        cautionSignals.push(`宿曜 ${context.xiu} 为凶`);
    if (POSITIVE_DAY_OFFICERS.has(context.dayOfficer))
        favorableSignals.push(`建除值星 ${context.dayOfficer} 偏利成事`);
    if (NEGATIVE_DAY_OFFICERS.has(context.dayOfficer))
        cautionSignals.push(`建除值星 ${context.dayOfficer} 偏需谨慎`);
    ...
    if (favorableSignals.length === 0)
        favorableSignals.push('主盘中性，宜结合问题轻重判断');
    if (cautionSignals.length === 0)
        cautionSignals.push('未见强烈阻滞信号，但仍需结合主诉审慎定性');
    return {
        favorableSignals,
        cautionSignals,
...
function buildJudgementAnchors(primaryStar, context, mode, minuteRefinement) {
    const summary = [
...
export function calculateTaiyi(input) {
