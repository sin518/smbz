...
function formatMappingReference(star) {
    return `${star.qimenName} / ${star.xuankongName}`;
}
function formatScaleLabel(star) {
    return star.scale === 'minute' ? '分钟主星' : star.scaleLabel;
}
function buildEnergyInteraction(result) {
...
export function renderTaiyiCanonicalJSON(result, options = {}) {
