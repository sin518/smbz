...
function analyzeTianGanWuHe(yearStem, monthStem, dayStem, hourStem, yearBranch, monthBranch, dayBranch, hourBranch) {
    ...
    for (let i = 0; i < stems.length; i++) {
        for (let j = i + 1; j < stems.length; j++) {
            ...
            if (element) {
                // 判断合化是否成立
                const { isTransformed, transformReason, notTransformReason } = checkHeHuaTransformation(element, i, j, branches, monthElement, stems);
                result.push({
                    stemA: stems[i],
                    stemB: stems[j],
 ...
 */
function checkHeHuaTransformation(huaShenElement, posA, posB, branches, monthElement, stems) {
    const reasons = [];
    const failReasons = [];
    // 条件1：化神当令或有强根
...
export function calculateBaziShenShaData(input) {
    ...
    try {
        ...
        warnings.push(`吉神数据获取失败: ${e instanceof Error ? e.message : String(e)}`);
    }
    try {
        xiongSha = lunar.getDayXiongSha() || [];
    }
    catch (e) {
        warnings.push(`凶煞数据获取失败: ${e instanceof Error ? e.message : String(e)}`);
    }
    try {
        dayYi = lunar.getDayYi() || [];
    }
    catch (e) {
        warnings.push(`日宜数据获取失败: ${e instanceof Error ? e.message : String(e)}`);
    }
    try {
        dayJi = lunar.getDayJi() || [];
    }
    catch (e) {
        warnings.push(`日忌数据获取失败: ${e instanceof Error ? e.message : String(e)}`);
    }
    return {
        jiShen,
        xiongSha,
