...
const ANGLE_KEYS = ['ascendant', 'midheaven'];
const MAJOR_ASPECT_TYPES = ['conjunction', 'opposition', 'trine', 'square', 'sextile'];
const SUPPORTED_ASPECT_KEYS = [...BODY_KEYS, ...POINT_KEYS, ...ANGLE_KEYS];
const BODY_LABELS = {
    sun: '太阳',
    moon: '月亮',
    mercury: '水星',
...
function ensureInteger(value, min, max, label) {
    if (!Number.isInteger(value) || value < min || value > max) {
        throw new Error(`${label} 必须是 ${min}-${max} 的整数`);
    }
}
function ensureValidCivilDate(year, month, day, label) {
...
function resolveLocationContext(input) {
    ...
    if (hasLatitude && hasLongitude) {
        ensureRange(input.latitude, -90, 90, 'latitude');
        ensureRange(input.longitude, -180, 180, 'longitude');
        return {
...
function parseTransitDateTime(value, derivedTimeZone) {
    if (!value) {
...
function buildFactor(entity, category) {
    return {
        key: entity.key,
...
function buildZodiacCusp(entity) {
    return {
        sign: buildSignInfo(entity.Sign),
        start: formatDegrees(entity.ChartPosition.Ecliptic.DecimalDegrees),
    };
}
function classifyFactorCategory(key) {
    if (ANGLE_KEYS.includes(key))
        return 'angle';
    if (POINT_KEYS.includes(key))
        return 'point';
    return 'body';
}
function normalizeAspectType(value) {
    return MAJOR_ASPECT_TYPES.includes(value)
        ? value
        : null;
}
function computeAngularDistance(left, right) {
    const diffRaw = Math.abs(left - right);
    const diff = diffRaw > 180 ? 360 - diffRaw : diffRaw;
    return Number(diff.toFixed(4));
}
function buildAspectFromLibrary(aspect, factorPositionByKey) {
...
function computeMajorAspect(from, to) {
...
function createHoroscope(origin, includeAngles) {
...
function buildHoroscopeSnapshot(origin, locationContext, birthPlace) {
    const includeAngles = locationContext.calculationMode === 'exact';
    const horoscope = createHoroscope(origin, includeAngles);
    const bodies = BODY_KEYS.map((key) => buildFactor(horoscope.CelestialBodies[key], 'body'));
    const points = POINT_KEYS.map((key) => buildFactor(horoscope.CelestialPoints[key], 'point'));
    const angles = includeAngles
        ? ANGLE_KEYS.map((key) => buildFactor(horoscope.Angles[key], 'angle'))
        : [];
    const houses = includeAngles
        ? horoscope.Houses.map((house) => buildHouse(house))
        : [];
    const zodiacCusps = horoscope.ZodiacCusps.map((cusp) => buildZodiacCusp(cusp));
    return {
        origin: {
...
function createNatalOrigin(input, locationContext) {
...
function createTransitOrigin(input, derivedTimeZone, locationContext) {
...
function sortAspects(aspects) {
    return [...aspects].sort((left, right) => {
        ...
        if (left.from.label !== right.from.label)
            return left.from.label.localeCompare(right.from.label, 'zh-CN');
        return left.to.label.localeCompare(right.to.label, 'zh-CN');
    });
}
export function calculateAstrology(input) {
    ...
    const natalFactorPositionByKey = new Map([...natal.bodies, ...natal.points, ...natal.angles].map((factor) => [factor.key, factor.position.decimal]));
    const majorAspects = sortAspects(natalHoroscope.Aspects.all
    ...
    const transitToNatalAspects = sortAspects(transitFactors.flatMap((from) => natalTargets.flatMap((to) => {
        if (from.key === to.key && from.category === to.category)
            return [];
