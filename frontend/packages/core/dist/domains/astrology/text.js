...
function formatFactorSummary(item, isApproximate) {
    const parts = [
        item.sign.label,
...
export function renderAstrologyCanonicalText(result, options = {}) {
    ...
    const lines = [
        ...
        '',
        '## 本命主星',
        ...model.natalBodies.map((entry) => `- ${entry.factor.label}: ${formatFactorSummary(entry.factor, isApproximate)} | 关键相位: ${entry.aspects.length > 0
