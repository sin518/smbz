...
function factorToSnapshotJson(item, isApproximate) {
    return {
        星座: item.sign.label,
        黄经: `${item.position.absolute} / ${item.position.withinSign}`,
        ...(!isApproximate && typeof item.house === 'number' ? { 宫位: `第${item.house}宫` } : {}),
        ...(item.retrograde ? { 逆行: '是' } : {}),
    };
}
function aspectToCompactJson(item, currentFactorKey) {
    return {
...
export function renderAstrologyCanonicalJSON(result, options = {}) {
