import type {
  DaliurenDerivationStep,
  DaliurenHarmCandidate,
  DaliurenHarmDepth,
  DaliurenNineGate,
  DaliurenOutput,
} from './types.js';
...
const STEM_HOME: Record<string, string> = {
  甲: '寅', 乙: '辰', 丙: '巳', 丁: '未', 戊: '巳',
  己: '未', 庚: '申', 辛: '戌', 壬: '亥', 癸: '丑',
};

const FIVE_ELEMENT: Record<string, string> = {
  甲: '木', 乙: '木', 寅: '木', 卯: '木',
  丙: '火', 丁: '火', 巳: '火', 午: '火',
...
const PALACE_HIDDEN_ELEMENTS: Record<string, string[]> = {
  亥: ['水', '水'],
  子: ['水'],
  丑: ['土', '水'],
  寅: ['木', '木'],
...
const STEM_COMBINATION: Record<string, string> = {
  甲: '己', 乙: '庚', 丙: '辛', 丁: '壬', 戊: '癸',
  己: '甲', 庚: '乙', 辛: '丙', 壬: '丁', 癸: '戊',
};

const THREE_HARMONY_FORWARD: Record<string, string> = {
  申: '子', 子: '辰', 辰: '申',
  亥: '卯', 卯: '未', 未: '亥',
  寅: '午', 午: '戌', 戌: '寅',
  巳: '酉', 酉: '丑', 丑: '巳',
...
type LessonLabel = typeof LESSON_LABELS[number];
type RelationKind = '下克上' | '上克下';
...
export function deriveNineGate(source: DerivationSource): NineGateDerivation {
  ...
  const steps: DaliurenDerivationStep[] = [];
  ...
  if (hasDirectControl) {
    if (lowerControlsUpper.length === 1) {
      ...
      steps.push(step(
        '贼克',
        `${priorityCandidates.length}个${priorityCandidates[0]?.relation || '克贼'}候选，进入比用裁决`,
        priorityCandidates,
      ));
      ...
      if (parityMatches.length === 1) {
        ...
        steps.push(step(
          '比用',
          `${dayGan}日取与日干阴阳相同的${directInitial}发用`,
          parityMatches,
        ));
      } else {
        ...
        if (harm.selected) {
          const harmStep = makeStep(
            '涉害',
            harm.depth.decision,
            harmCandidates.map((candidate) => candidate.lesson.up),
          );
          steps.push(harmStep);
          if (!overlap) {
            if (reverse) {
  ...
  if (reverse && !hasDirectControl) {
    const transmissions: [string, string, string] = [
      HORSE[dayZhi] || '',
      lessons[2]?.up || '',
      lessons[0]?.up || '',
    ];
    steps.push(makeStep(
      '反吟',
      `天地盘相冲且四课无克，取日支驿马${transmissions[0]}发用`,
      [transmissions[0]],
    ));
  ...
  if (uniqueLessonCount === 2) {
    ...
    const transmissions: [string, string, string] = [initial, lessons[0].up, lessons[0].up];
    steps.push(makeStep(
      '八专',
      `${yangDay ? '阳日顺数' : '阴日逆数'}三位取${initial}发用，中末传同取干上${lessons[0].up}`,
      [initial],
    ));
  ...
  if (uniqueLessonCount === 3 || uniqueLessonCount === 4) {
    ...
    if (remote) {
      ...
      steps.push(makeStep(
        '遥克',
        `${subtype}：${remote.lesson.up}${subtype === '蒿矢' ? '遥克日干' : '受日干遥克'}，取其发用`,
        [remote.lesson.up],
      ));
  ...
  if (uniqueLessonCount === 4) {
    const yangDay = isYang(dayGan);
    const transmissions: [string, string, string] = yangDay
      ? [
    ...
    steps.push(makeStep(
      '昴星',
      `${yangDay ? '阳日虎视' : '阴日冬蛇掩目'}，取${transmissions[0]}发用`,
      [transmissions[0]],
    ));
  ...
  if (uniqueLessonCount === 3) {
    ...
    steps.push(makeStep(
      '别责',
      `${yangDay ? '阳日取日干五合寄宫上神' : '阴日取日支三合前神'}${initial}发用，中末传同取干上${lessons[0].up}`,
      [initial],
    ));
...
function deriveOverlap(
  source: DerivationSource,
  directInitial: string,
  ...
  if (final === middle || final === initial) final = OPPOSITE[middle] || '';
  const transmissions: [string, string, string] = [initial, middle, final];
  steps.push(makeStep(
    '伏吟',
    `${hasDirectControl ? '有克先取克贼发用' : `${isYang(dayGan) ? '阳日取干上' : '阴日取支上'}发用`}，中末传依刑、冲递取`,
    transmissions,
  ));
  return { method: '伏吟', gate: '伏吟', transmissions, steps, harmDepth: null };
}

function deriveHarmDepth(
  source: DerivationSource,
  candidates: Candidate[],
  dayGan: string,
): { selected: string; depth: DaliurenHarmDepth } {
  ...
  let tied = [...analyses];
  let subtype: DaliurenHarmDepth['subtype'] = '涉害';
...
function buildHarmCandidate(source: DerivationSource, candidate: Candidate): DaliurenHarmCandidate {
  const branch = candidate.lesson.up;
  const start = groundUnderSky(source, branch);
  const path: DaliurenHarmCandidate['path'] = [];
  let ground = start;
  ...
  for (let stepIndex = 0; stepIndex < BRANCHES.length; stepIndex += 1) {
    ...
    const hitCount = hiddenElements.filter((hiddenElement) => (
      candidate.relation === '下克上'
        ? controls(hiddenElement, branchElement)
        : controls(branchElement, hiddenElement)
    )).length;
    path.push({ groundBranch: ground, hiddenElements: [...hiddenElements], hitCount });
    if (ground === branch) break;
...
function buildLessons(source: DerivationSource): Lesson[] {
...
function uniqueCandidates(lessons: Lesson[], relation: RelationKind): Candidate[] {
  const matches = lessons.filter((lesson) => (
    relation === '下克上'
      ? controls(FIVE_ELEMENT[lesson.down], FIVE_ELEMENT[lesson.up])
      : controls(FIVE_ELEMENT[lesson.up], FIVE_ELEMENT[lesson.down])
  )).map((lesson) => ({ lesson, relation }));
  return uniqueByBranch(matches);
}

function uniqueByBranch(candidates: Candidate[]) {
  const result: Candidate[] = [];
  for (const candidate of candidates) {
    if (!result.some((item) => item.lesson.up === candidate.lesson.up)) result.push(candidate);
  }
  return result;
}

function selectByParity(candidates: Candidate[], dayGan: string) {
...
function step(gate: DaliurenNineGate, summary: string, candidates: Candidate[]) {
  return makeStep(gate, summary, candidates.map((candidate) => `${candidate.lesson.label}${candidate.lesson.up}`));
}

function makeStep(gate: DaliurenNineGate, summary: string, candidates: string[]): DaliurenDerivationStep {
  return { gate, summary, candidates };
}

function skyAtGround(source: DerivationSource, branch: string) {
  const index = BRANCHES.indexOf(branch as typeof BRANCHES[number]);
  return index >= 0 ? source.tianDiPan.tianPan[String(index)] || '' : '';
}

function groundUnderSky(source: DerivationSource, branch: string) {
...
function offsetBranch(branch: string, offset: number) {
  const index = BRANCHES.indexOf(branch as typeof BRANCHES[number]);
  if (index < 0) return '';
...
function isYang(value: string) {
  return YANG.has(value);
}

function formatDepths(candidates: DaliurenHarmCandidate[]) {
  return candidates.map((candidate) => `${candidate.branch}${candidate.depth}重`).join('、');
}

function normalizeGate(method: string): DaliurenNineGate {
  if (method.includes('伏吟')) return '伏吟';
  if (method.includes('返吟') || method.includes('反吟')) return '反吟';
  if (method.includes('八专')) return '八专';
  if (method.includes('别责')) return '别责';
  if (method.includes('昴星')) return '昴星';
  if (method.includes('遥克') || method.includes('蒿矢') || method.includes('弹射')) return '遥克';
  if (method.includes('涉害') || method.includes('见机') || method.includes('察微') || method.includes('缀瑕')) return '涉害';
  if (method.includes('比用') || method.includes('知一')) return '比用';
  return '贼克';
}
