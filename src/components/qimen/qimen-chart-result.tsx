    ...
    <main className="mx-auto min-h-screen max-w-[430px] bg-[#f6f5ef] pb-8 text-ink shadow-soft">
      <header className="px-5 pb-2 pt-8">
        <div className="flex items-center justify-between">
          <Link href="/qimen" className="-ml-2 flex h-10 w-10 items-center justify-center" aria-label="返回奇门遁甲">
            <ArrowLeft size={24} />
          </Link>
          <h1 className="text-[24px] font-semibold">奇门排盘</h1>
      ...
      <div className="space-y-4 px-4 pt-4">
        ...
        {result ? <QimenChartCard chart={result.chart} /> : null}

        {loaded && !result ? (
          <section className="rounded-[22px] bg-white p-5 text-center shadow-soft">
            <p className="text-[18px] font-semibold">还没有排盘结果</p>
            <p className="mt-2 text-[14px] leading-6 text-[#7d7972]">返回填写页，提交后会在这里展示九宫盘。</p>
            <Link href="/qimen" className="mt-4 inline-flex h-11 items-center rounded-lg bg-[#c91f08] px-6 text-[17px] font-semibold text-white">
              去排盘
            </Link>
          </section>
        ) : null}
...
function ResultInfoPanel({ result }: { result: StoredQimenResult }) {
  ...
  return (
    <section className="border-b border-[#e7e2d6] px-1 pb-4 pt-1 text-[12px] font-semibold leading-7 text-[#3b3935]">
      <div className="grid grid-cols-2 gap-x-4">
        <InfoLine label="姓名" value={input?.name?.trim() || "未填写"} />
        <InfoLine label="性别" value={input?.gender === "female" ? "女" : "男"} />
      </div>
      <InfoLine label="起局时间" value={`${formatChineseSolar(chart.solarText)} ${chart.lunarText}`} compact />
...
function InfoLine({ label, value, compact }: { label: string; value: string; compact?: boolean }) {
  return (
    <p className={compact ? "min-w-0 whitespace-nowrap" : "min-w-0"}>
      <span className="text-[#77746d]">{label}：</span>
      <span className={compact ? "" : "break-words"}>{value}</span>
    </p>
  );
}
...
function QimenChartCard({ chart }: { chart: QimenChart }) {
  return (
    <section className="rounded-[22px] bg-white p-3 shadow-soft">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="px-1 text-[22px] font-semibold">九宫盘</h2>
        <span className="rounded-full bg-[#f4f1ed] px-3 py-1 text-[13px] font-medium text-[#a58024]">
          {`${chart.plateType === "zhuan" ? "转盘" : "飞盘"} · ${chart.dunType === "yang" ? "阳遁" : "阴遁"}${chart.ju}局`}
        </span>
      </div>

      <div className="overflow-hidden rounded-md border border-[#eaded2] bg-[#fffdf7]">
        <div className="grid grid-cols-4 bg-[#c32012] text-center text-[12px] font-semibold leading-8 text-white">
          <span>年柱：{formatPillar(chart.pillars.year)}</span>
          <span>月柱：{formatPillar(chart.pillars.month)}</span>
          <span>日柱：{formatPillar(chart.pillars.day)}</span>
          <span>时柱：{formatPillar(chart.pillars.hour)}</span>
        </div>
        <div className="grid grid-cols-4 border-b border-[#eaded2] bg-[#fff9ed] px-2 py-2 text-[12px] font-semibold text-[#c32012]">
          <span>{chart.dunType === "yang" ? "阳遁" : "阴遁"}{chart.ju}局</span>
          <span>旬首：{chart.hourXun.leader}</span>
          ...
          <div key={palace.number} className="relative min-h-[118px] border border-[#eaded2] p-2 pb-6 text-[12px] leading-[18px] text-[#2f2d2a]">
            ...
            <div className="absolute bottom-2 left-2 right-2 flex items-end justify-between text-[12px]">
              ...
              <span className="flex flex-col items-end leading-4">
                ...
                <span className="font-semibold text-[#c32012]">{palace.earthStem}</span>
              </span>
            </div>
          </div>
        ))}
...
function StatusBadge({ value }: { value: string | null }) {
  ...
  return (
    <span className="absolute right-2 top-2 bg-[#c32012] px-1 py-0.5 font-semibold text-white">
      {value}
    </span>
  );
}
