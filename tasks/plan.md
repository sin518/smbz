...
# Implementation Plan: 四模块 AI 解读分层浏览

## Overview

把大六壬已经验证的“快速浏览＋完整依据”输出协议同步到奇门、紫微、六爻和八字。统一移动端阅读方式、去重和篇幅原则，同时保留各模块既有的数据边界与专业判断主线；不修改排盘算法、页面、接口或数据库。

## Architecture Decisions

- 统一阅读骨架，不共享一份通用提示词：四个模块分别保留宫位、用神、旺衰、运限等领域顺序。
- 快速浏览固定覆盖一句话结论、领域主线、时间线索和行动建议；完整依据保留全部独立事实、支持与反向证据。
- 不依赖外部 AI 对 HTML 折叠的支持；只使用稳定的 Markdown 标题、短段落和项目符号。
- 每个模块增加独立提示词回归测试，锁定标题、去重、移动端排版和原有安全边界。

## Task List

### Phase 1: Contract Tests

- [x] Task 1：为奇门、紫微、六爻和八字补充或更新分层输出协议测试。

### Checkpoint: Contract

- [x] 四个模块的旧提示词均因尚未满足新协议而出现预期测试失败。

### Phase 2: Module Adoption

- [x] Task 2：将奇门与紫微提示词改为领域定制的“快速浏览＋完整依据”。
- [x] Task 3：将六爻与八字提示词改为领域定制的“快速浏览＋完整依据”。

### Checkpoint: Adoption

- [x] 四模块提示词测试全部通过，原有数据边界与高风险事项规则仍被测试覆盖。

### Phase 3: Verification

- [x] Task 4：运行相关测试、类型检查、Lint、生产构建和差异检查。

### Checkpoint: Complete

- [x] 所有验收通过；没有修改排盘算法、页面、接口、数据库或用户无关文件。

## Risks and Mitigations

| Risk | Impact | Mitigation |
|---|---|---|
| 统一结构时抹平领域差异 | 高 | 每个模块保留独立主线与详情分组，不抽取通用提示词正文 |
| 八字提示词体量大且缺少测试 | 高 | 先补契约测试，再只改输出结构和重复表达规则 |
| 六爻现有四段结构被过度扩写 | 中 | 保留原有结论、依据、应期、边界，新增简短快速浏览而非增加论证 |
| 外部模型不完全遵循 Markdown | 中 | 使用明确标题、固定字段与自查规则，不依赖 HTML 或应用内解析 |

## Open Questions

- 无；产品结构已经由 Grill 审查逐项确认。
