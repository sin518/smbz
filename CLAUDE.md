...
- `supabase/table_export_from_supabase.sql`: 当前数据库 schema 导出快照。
...
- 新增表/字段前，必须先检查 `supabase/table_export_from_supabase.sql` ，并说明”为何不能复用现有结构”，必要时可以检查 `mcp supabase migrations`。
